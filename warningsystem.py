            ...
            for line in f:
                ...
                if line and line[0] != '#' and (',' in line or '=' in line):
                    if ',' in line:
                        ...
                        continue
                    if item in 'camera_id camera_width camera_height camera_frame_rate camera_fourcc auto_percent ' \
                               'auto_threshold auto_blur norm_alpha norm_beta' \
                            .split():
                        try:
                            exec(f'{item}={value}')
                    ...
                    camera.camera_source = 0  # camera_id #SET THE CORRECT CAMERA NUMBER
                    ...
                    # distance unit designator
                    unit_suffix = 'mm'
                    # calibrate every N pixels
                    pixel_base = 10
                    # maximum field of view from center to the farthest edge(should be measured in unit_suffix)
                    cal_range = 114
                    ...
                    # calibration loop values inside of main loop below
                    cal_base = 5
                    cal_last = None

                    # calibration update
                    def cal_update(x, y, unit_distance, baseround=None):
                        # basics
                        ...
                        framename = "object measure"
                        cv2.namedWindow(framename, flags=cv2.WINDOW_NORMAL | cv2.WINDOW_GUI_NORMAL)
                        # what to happen when you press the letters a,c,r......
                        key_last = 0
                        ...
                        def key_event(key):
                            ...
                            # auto percent
                                ...
                                key_flags['percent'] = not key_flags['present']
                                key_flags['thresh'] = False
                                key_flags['lock'] = False
                            # auto threshold
                        ...
                        # mouse callback
                        def mouse_event(event, x, y, flags, parameters, cal_last=None):
                            ...
                            # update threshold
                                ...
                                auto_threshold = int(225 * x / width)
                                auto_blur = int(20 * y / height) | 1  # insure it is odd and atleast 1
                            ...
                            while 1:
                                # get frame
                                frame0: object = camera.next(wait=1)
                                if frame0 is None:
                                    time.sleep(0, 1)
                                    continue
                                # normalize
                                cv2.normalize(frame0, frame0, norm_alpha, norm_beta, cv2.NORM_MINIMAX)
                                # rotate 180
                                if key_flags['rotate']:
                            ...
                            # auto mode
                                ...
                                for c in contours:
                                    ...
                                    x3, y3 = x1 + (w / 2), y1 + (h / 2)
                                    # percent area
                                    auto_percent = 100 * w * h / area
                                    # if the contour is too small, ignore it
                                    if percent < auto_percent:
                                        continue
                                        # convert to center, the distance
                                        ...
                                        if max(xlen, ylen) > 0 and min(xlen, ylen) / max(xlen, ylen) >= 0.95:
                                            ...
                                            # add dimensions
                                            draw.add_text(frame0, f'(xlen:2f)', x1 - ((x1 - x2) / 2), min(y1, y2) - 8,
                                                          center=True, color='red')
                                            draw.add_text(frame0, f'Area:{carea:.2f}', x3, y2 + 8, center=True,
                                                          top=True, color='red')
                                            if alen:
                                                draw.add_text(frame0, f'Avg:{alen:.2f}', x3, y2 + 34, center=True,
                                                              top=True, color='green')
                                                if x1 < width - x2:
                                                    draw.add_text(frame0, f'{ylen:.2f}', x2 + 4, (y1 + y2) / 2,
                                                                  middle=True, color='red')
                                                else:
                                                    draw.add_text(frame0, f'{ylen:.2f}', x1 - 4, (y1 + y2) / 2,
                                                                  middle=True, right=True, color='red')
                                                    # alert for size above specified value
                                                    if ylen > 25:
                                                        text.append('')
                                                        text.append('Exceeds value!!')
                                                        alert("Heavy vehicle detected")
                                            # manual mode
                                            else:
                                                # small crosshairs
                                                draw.crosshairs(frame0, 5, weight=2, color='green')
                                                ...
                                                if mouse_mark:
                                                    ...
                                                    alen = 0
                                                    if max(xlen, ylen) > 0 and min(xlen, ylen) / max(xlen,
                                                                                                     ylen) >= 0.95:
                                                        alen = (xlen + ylen) / 2
                                                    carea = xlen * ylen
                                                    # print distances
                                                    ...
                                                    draw.add_text(frame0, f'Area:{carea:.2f}', x3, y3 + 8, center=True,
                                                                  top=true, color='red')
                                                    if alen:
                                                        draw.add_text(frame0, f'Avg:{alen:.2f}', x3, y3 + 34,
                                                                      center=True, top=True, color='green')
                                                    if x2 <= x1:
                                                        draw.add_text(frame0, f'{ylen:.2f}', x1 + 4, (y1 + y2) / 2,
                                                                      middle=True, color='red')
                                                        draw.add_text(frame0, f'{llen:.2f}', x2 - 4, y2 - 4, right=True,
                                                                      color='green')
                                                    else:
                                                        draw.add_text(frame0, f'{ylen:.2f}', x1 - 4, (y1 + y2) / 2,
                                                                      middle=True, right=True, color='red')
                                                        draw.add_text(frame0, f'{llen:.2f}', x2 + 8, y2 - 4,
                                                                      color='green')
                                                    if ylen > 25:
                                                        text.append('')
                                                        text.append('Exceeds Value!!')
